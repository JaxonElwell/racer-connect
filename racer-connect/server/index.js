...
app.get('/api/StudentOrganizations', (req, res) => {
    const { id } = req.params;
    db.all('SELECT * FROM StudentOrganizations WHERE id = ?', [id], (err, row) => {
        if(err) {
...
app.post('/api/StudentOrganizations', (req, res) => {
    ...
    if (!name || !category) {
        ...
        const sql = 'INSERT INTO StudentOrganization(name, category, president, president_email, advisor, advisor_email) VALUES (?, ?, ?, ?, ?, ?)'
